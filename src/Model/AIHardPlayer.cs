/// <summary>
/// AIHardPlayer is a type of player. This AI will know directions of ships when it has found 2 ship
/// tiles and will try to destroy that ship. If that ship is not destroyed it will shoot the other
/// way. Ship still not destroyed, then the AI knows it has hit multiple ships. Then will try to
/// destoy all around tiles that have been hit.
/// </summary>
using System;
...
namespace MyGame
{
    public class AIHardPlayer : AIPlayer
    {
        ...
        /// <summary>
        /// Private enumarator for AI states. currently there are two states, the AI can be searching
        /// for a ship, or if it has found a ship it will target the same ship
        /// </summary>
        ...
        /// <summary>
        /// SearchCoords will randomly generate shots within the grid as long as its not hit that
        /// tile already
        /// </summary>
        ...
        /// <summary>
        /// ProcessShot is able to process each shot that is made and call the right methods
        /// belonging to that shot. For example, if its a miss = do nothing, if it's a hit = process
        /// that hit location
        /// </summary>
        ...
        private void ProcessDestroy(int row, int col, Ship ship)
        {
            ...
            for (i = 1; i <= ship.Hits - 1; i++)
            {
                if (!foundOriginal)
                {
                    ...
                    // Source is nnothing if the ship was originally hit in the middle. This then
                    // searched forward, rather than backward through the list of targets
                    if (source == null)
                    {
        ...
        /// the source of the targets belong to the destroyed ship. If they don't put them on a new
        /// stack. Then clear the targets stack and move all the targets that still need to be shot
        /// at back onto the targets stack
        /// </summary>
        ...
        /// <summary>
        /// ProcessHit gets the last hit location coordinates and will ask AddTarget to create
        /// targets around that location by calling the method four times each time with a new
        /// location around the last hit location. It will then set the state of the AI and if it's
        /// not Searching or targetingShip then start ReOrderTargets.
        /// </summary>
        ...
        /// <summary>
        /// ReOrderTargets will optimise the targeting by re-orderin the stack that the targets are
        /// in. By putting the most important targets at the top they are the ones that will be shot
        /// at first.
        /// </summary>
        private void ReOrderTargets()
        {
            ...
            if (_CurrentTarget.SameRow)
                ...
                // else if the ship is lying on the same column, call MoveToTopOfStack to optimise on
                // the column
                MoveToTopOfStack(-1, _CurrentTarget.ShotAt.Column);
        }

        /// <summary>
        /// MoveToTopOfStack will re-order the stack by checkin the coordinates of each target If
        /// they have the right column or row values it will be moved to the _Match stack else put it
        /// on the _NoMatch stack. Then move all the targets from the _NoMatch stack back on the
        /// _Targets stack, these will be at the bottom making them less important. The move all the
        /// targets from the _Match stack on the _Targets stack, these will be on the top and will
        /// there for be shot at first
        /// </summary>
